...
// Converted to double-buffering by extending JApplet
public class BadDoubleBufferNarcotic extends Applet implements MouseMotionListener, MouseListener{
	
	/** Double Buffering Technique. */
	Graphics offscreenGraphics; // object to handle off-screen drawing
	Image    offscreenImage;    // image that contains 
	
	/** Make sure that image is created as needed. */
	boolean ensureImageAvailable(Graphics g) {
	...
	public void redraw() {
		...
		// Note: This was the original paint(Graphics) implementation, now just
		// rewritten to use offscreenGraphics instead of g.
		if (idx != 52) offscreenGraphics.drawImage(cards[52], 10, 10, this);
		for (int i = 0; i < 4; i++) {
			if (tp[i] < 0) continue;
			int x = pt[i][tp[i]];
			if (x >= 0)	offscreenGraphics.drawImage(cards[x], 20+(i+1)*80, 10, this);
		}
		if (z != -1) offscreenGraphics.drawImage(cards[z], p.x-ax, p.y-ay, this);
		offscreenGraphics.drawString(""+sc, 500, 10);
		offscreenGraphics.drawString(""+nl, 500, 80);
	...
	public void init() {
		...
		for (StringTokenizer rst = new StringTokenizer(p1); rst.hasMoreTokens(); ) {
			...
			for (StringTokenizer sst = new StringTokenizer(p2); sst.hasMoreTokens(); ) {
				...
				java.net.URL url = this.getClass().getResource ("/images/oxymoron/" + key + ".gif");
				cards[b] = java.awt.Toolkit.getDefaultToolkit().getImage(url);
				mt.addImage (cards[b], b++);
			}
		}
		java.net.URL u = this.getClass().getResource ("/images/oxymoron/Back.gif");
	...
	public void paint (Graphics g) {
		if (!ensureImageAvailable(g)) { return; }
